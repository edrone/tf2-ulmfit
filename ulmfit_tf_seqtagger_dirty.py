...
from modelling_scripts.ulmfit_tf2_heads import ulmfit_sequence_tagger, ulmfit_fake_tagger
...
def tokenize_and_align_labels(spmproc, ddpl_iob):
    ...
    for sent in ddpl_iob:
        ...
        sentence_tokens = sentence_tokens[:198] # fixme: this should be a parameter. with bsize=32 we get GPU OOM errors
        sentence_ids = sentence_ids[:198] # fixme: this should be a parameter. with bsize=32 we get GPU OOM errors
        sentence_labels = sentence_labels[:198] # fixme: this should be a parameter. with bsize=32 we get GPU OOM errors
...
def train_step(model, loss_fn, optimizer, x, y, step_number): # todo: https://www.tensorflow.org/guide/keras/customizing_what_happens_in_fit
    ...
    with tf.profiler.experimental.Profile('/home/hkarbowy/tmp/logowanie'):
...
def main(args):
    ...
    ######## VERSION 1: Uncomment this block to build a plain LSTM model (only standard Keras objects)
    #ulmfit_tagger = ulmfit_fake_tagger(num_classes=3,
    #                                   pretrained_weights=args['model_weights_cp'],
    #                                   spm_model_file=args['spm_model_file'],
    #                                   fixed_seq_len=200,
    #                                   also_return_spm_encoder=False)

    ######## VERSION 2: ULMFiT model built from Python code - pass the path to 'wagi_ragged' checkpoint in model_weights_cp
    ...
    ######## VERSION 3: ULMFiT model serialized to a SavedModel - pass the path to 'wyeksportowany200' as model_weights_cp
    ...
    loss_fn_ragged = tf.keras.losses.SparseCategoricalCrossentropy()
    optimizer = tf.keras.optimizers.Adam()
    # tboard_cb = tf.keras.callbacks.TensorBoard(log_dir='/home/hkarbowy/tmp/logowanie', update_freq='batch', profile_batch=(2,5))
    ...
    ulmfit_tagger.fit(sequence_inputs, subword_labels, epochs=1, batch_size=args['batch_size'], callbacks=[])

    ############## ALTERNATIVELY USE THE GRADIENT TAPE ###########################################
