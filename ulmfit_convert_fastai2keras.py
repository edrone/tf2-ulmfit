...
def main(args):
    ...
    if args.get('fixed_seq_len') is None:
        ...
        tf.saved_model.save(exportable, os.path.join(args['out_path'], 'saved_model', exp_name), signatures=convenience_signatures)
...
if __name__ == "__main__":
    ...
    argz.add_argument("--awd-weights", choices=['on', 'off'], default='on', help="Whether to save weights for the AWD-LSTM cell (on) or the plain variant (off)")
    argz.add_argument("--fixed-seq-len", type=int, required=False, help="Fixed sequence length. If unset, the RNN encoder will output ragged tensors.")
    argz.add_argument("--spm-model-file", required=True, help="Fixme: This isn't really necessary")
